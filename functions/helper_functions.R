...
local_maxima <- function(beat_df, MSNA_df, latency, t_window) {
  ...
  for (i in seq_along(beat_df$time)) {
   #i<-645
    temp2 <- MSNA_df %>% 
      filter(MSNA_time >= beat_df$time[[i]] + (latency - t_window) & MSNA_time <= beat_df$time[[i]] + (latency + t_window))
...
window <- function(df, MSNA, fs) {
  ...
  for (i in seq_along(df$beat_no)) {
    temp2 <- MSNA %>% 
      filter(MSNA_time >= df$t_max[[i]] - df$RRI[[i]]/2 & MSNA_time <= df$t_max[[i]] + df$RRI[[i]]/2)
...
start_end <- function(x, BurstThreshold, SNR) {
  ...
  # Determine start details from deriv_times <0
    start_df <- x %>% filter(deriv_time <= 0)
      
    if (1 %in% start_df$updn_1d) {
    
      start <- start_df %>% filter(updn_1d > 0) %>% .[nrow(.),] %>% cbind("POI" = "start", .)
  ...
  end_df <- x %>% filter(deriv_time >= 0)
  
    if (1 %in% end_df$updn_1d) {
      
      end <- end_df %>% filter(updn_1d > 0) %>% .[1,] %>% cbind("POI" = "end", .)
...
burst_character <- function(x) {
  ...
  start_t <- start_end %>% filter(POI == "start") %>% select(deriv_time) %>% .[[1]]
  end_t <- start_end %>% filter(POI == "end") %>% select(deriv_time) %>% .[[1]]
  
  start_v <- start_end %>% filter(POI == "start") %>% select(MSNA_v_zeroed) %>% .[[1]]
  rise_amp <- MSNA_max - start_v
  rise_slope <- rise_amp/abs(start_t)
  
  end_v <- start_end %>% filter(POI == "end") %>% select(MSNA_v_zeroed) %>% .[[1]]
  fall_amp <- MSNA_max - end_v
  fall_slope <- fall_amp/abs(end_t)
  
  # determine burst area
  temp <- burst_df %>% filter(rel_time >= start_t & rel_time <= end_t) %>% mutate(MSNA_v_zeroed = MSNA_v_zeroed - min(start_v, end_v))
